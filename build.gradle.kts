...
group = "dev.redstudio"
version = "{FIRST VERSION}-Dev-1" // Versioning must follow Ragnarök versioning convention: https://shor.cz/ragnarok_versioning_convention

val id = "{MAIN PACKAGE}"
val plugin = "asm.{ID WITH UPERCASE LETTERS}Plugin"
...
buildConfig {
	...
	documentation.set("This class defines constants for {NAME WITH SPACES}.\n<p>\nThey are automatically updated by Gradle")

	useJavaOutput()
	buildConfigField("String", "ID", provider { """"${id}"""" })
	buildConfigField("String", "NAME", "\"{NAME WITH SPACES}\"")
	buildConfigField("String", "VERSION", provider { """"${project.version}"""" })
	buildConfigField("org.apache.logging.log4j.Logger", "LOGGER", "org.apache.logging.log4j.LogManager.getLogger(NAME)")
	buildConfigField("dev.redstudio.redcore.logging.RedLogger", "RED_LOGGER", """new RedLogger(NAME, "{NO NEW ISSUE LINK}", LOGGER)""")
}
